...
class Contacto:
    ...
    def __init__(self, nombre, apellido, empresa, email, movil):
...
class Agenda(Patr_Obs):
    ...
    def cargar_contacto(self, nombre, apellido, empresa, email, movil, image_64_encode):
        ...
        if askyesno("Consulta", "¿Desea dar de alta el contacto?"):
            sql = "INSERT INTO contactos (NOMBRE,APELLIDO,EMPRESA,EMAIL,TELEFONO,FOTO)VALUES(%s,%s,%s,%s,%s,%s)"
            val = [nombre, apellido, empresa, email, movil, image_64_encode]
            operacion_db(sql, val)
            showinfo("OK", "Operación exitosa")
            contacto = Contacto(nombre, apellido, empresa, email, movil)
            self.contactos.append(contacto)
            self.notificar()

    def cargar_csv(self, nombre, apellidos, empresa, email, movil):

        """ esta funcion carga la tabla en el .csv al exportar """

        contacto = Contacto(nombre, apellidos, empresa, email, movil)
        self.contactos.append(contacto)
    ...
    def grabar(self):
        ...
        with open("Archivo-CSV.csv", "w") as fichero:
            ...
            escribir.writerow(
                ("codigo", "nombre", "apellidos", "empresa", "email", "movil")
            )
...
def exportar():
    ...
    if a != 0:
        for x in resultado:
            ...
            agenda.cargar_csv(nombre, apellido, empresa, email, movil)
            agenda.grabar()
